...
export const initiateOutboundCall = async (payload: CallPayload): Promise<VapiCallResponse> => {
  ...
  if (!response.ok) {
    ...
    const vapiMsg = (errorData as any)?.message || "";
    if (response.status === 401) {
      throw Object.assign(new Error("Vapi API key is invalid. Please check your VAPI_API_KEY in .env."), { statusCode: 401 });
    }
    if (vapiMsg.includes("international calls")) {
      throw Object.assign(new Error("Free Vapi numbers do not support international calls. Please upgrade your Vapi plan or use a local number."), { statusCode: 400 });
    }
